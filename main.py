...
class DBConnection:
    """Обёртка над sqlite3 соединением с поддержкой контекстного менеджера."""
    def __init__(self):
...
def log_msg(user_id, chat_id, msg_id, is_admin_side=False):
    ...
    try:
        ...
        with get_db_connection() as conn:
            c = conn.cursor()
            c.execute('INSERT INTO msg_log (user_id, chat_id, msg_id, is_admin_side, created_at) VALUES (?,?,?,?,?)',
...
def get_logged_msgs(user_id, is_admin_side=False):
    ...
    try:
        with get_db_connection() as conn:
            c = conn.cursor()
            return c.execute(
                'SELECT chat_id, msg_id FROM msg_log WHERE user_id=? AND is_admin_side=?',
...
def clear_msg_log(user_id):
    ...
    try:
        with get_db_connection() as conn:
            c = conn.cursor()
            c.execute('DELETE FROM msg_log WHERE user_id=?', (user_id,))
    except Exception as e:
...
def add_product_to_db(key, name, price):
    try:
        with get_db_connection() as conn:
            ...
            c.execute('INSERT INTO products (key, name, price, sort_order) VALUES (?,?,?,?) ON CONFLICT(key) DO UPDATE SET name=excluded.name, price=excluded.price, sort_order=excluded.sort_order',
...
def user_exists(user_id):
    try:
        with get_db_connection() as conn:
            c = conn.cursor()
            row = c.execute('SELECT user_id, first_name, username FROM users WHERE user_id=?', (user_id,)).fetchone()
...
def add_pending_key(product_key, key_value, added_by):
    try:
        ...
        with get_db_connection() as conn:
            c = conn.cursor()
            c.execute('INSERT INTO pending_keys (product_key, key_value, added_by, added_at) VALUES (?,?,?,?)',
...
def get_pending_keys_by_group(group_id):
    ...
    try:
        with get_db_connection() as conn:
            c = conn.cursor()
            return c.execute('SELECT id, product_key, key_value, added_by FROM pending_keys WHERE id>=? ORDER BY id',
                             (group_id,)).fetchall()
    except Exception as e:
...
def approve_pending_key_group(first_id, last_id):
    ...
    try:
        with get_db_connection() as conn:
            c = conn.cursor()
            rows = c.execute('SELECT product_key, key_value FROM pending_keys WHERE id BETWEEN ? AND ?',
                             (first_id, last_id)).fetchall()
            for product_key, key_value in rows:
                c.execute('INSERT INTO product_keys (product_key, key_value) VALUES (?,?)', (product_key, key_value))
            c.execute('DELETE FROM pending_keys WHERE id BETWEEN ? AND ?', (first_id, last_id))
            return len(rows)
    except Exception as e:
...
def reject_pending_key_group(first_id, last_id):
    ...
    try:
        with get_db_connection() as conn:
            c = conn.cursor()
            cnt = c.execute('SELECT COUNT(*) FROM pending_keys WHERE id BETWEEN ? AND ?',
...
def atomic_purchase(user_id, pool_key, price, item_id, item_name):
    """
    Атомарная покупка: замок + SQLite транзакция.
    ...
    with _purchase_lock:
        ...
        try:
            ...
            if not key_row:
                ...
                return None, 'no_key'
            now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            c.execute('UPDATE product_keys SET is_used=1, used_at=?, buyer_id=? WHERE id=?',
...
def get_user_order_count(user_id):
    try:
        with get_db_connection() as conn:
            c = conn.cursor()
            return c.execute('SELECT COUNT(*) FROM orders WHERE user_id=?', (user_id,)).fetchone()[0]
    except Exception as e:
...
def get_user_review_count(user_id):
    try:
        with get_db_connection() as conn:
            c = conn.cursor()
            return c.execute('SELECT COUNT(*) FROM reviews WHERE user_id=?', (user_id,)).fetchone()[0]
    except Exception as e:
...
def delete_user_reviews(user_id):
    try:
        with get_db_connection() as conn:
            c = conn.cursor()
            cnt = c.execute('SELECT COUNT(*) FROM reviews WHERE user_id=?', (user_id,)).fetchone()[0]
...
def save_review(user_id, username, item_name, review_text, photo_file_id=None, status='pending'):
    try:
        with get_db_connection() as conn:
            ...
            now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            c.execute(
                'INSERT INTO reviews (user_id, username, item_name, review_text, photo_file_id, status, date) VALUES (?,?,?,?,?,?,?)',
...
def approve_review(review_id):
    try:
        with get_db_connection() as conn:
            ...
            return c.execute(
                'SELECT user_id, username, item_name, review_text, photo_file_id FROM reviews WHERE id=?', (review_id,)
            ).fetchone()
    except Exception as e:
...
def reject_review(review_id):
    try:
        with get_db_connection() as conn:
            c = conn.cursor()
            c.execute("UPDATE reviews SET status='rejected' WHERE id=?", (review_id,))
            return c.execute(
                'SELECT user_id, username, item_name, review_text FROM reviews WHERE id=?', (review_id,)
            ).fetchone()
    except Exception as e:
...
def join_giveaway(giveaway_id, user_id):
    try:
        with get_db_connection() as conn:
            c = conn.cursor()
            c.execute('SELECT 1 FROM giveaway_participants WHERE giveaway_id=? AND user_id=?', (giveaway_id, user_id))
            if c.fetchone() is not None:
                return False
            c.execute('INSERT OR IGNORE INTO giveaway_participants (giveaway_id, user_id) VALUES (?, ?)', (giveaway_id, user_id))
            return True
    except Exception as e:
...
def db_add_manager(user_id):
    try:
        with get_db_connection() as conn:
            c = conn.cursor()
            c.execute('INSERT OR IGNORE INTO managers (user_id, added_at) VALUES (?, ?)',
...
if __name__ == '__main__':
    print("[INIT] Проверяю подключение к SQLite...")
    try:
        with get_db_connection() as _test_conn:
            pass
        print("[INIT] База данных доступна.")
    except Exception as e:
        print(f"[INIT] Ошибка базы данных: {e}")
    init_db()
    reload_managers()
